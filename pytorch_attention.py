...
from snp_input import  Tokenised_SNVs
...
class ExplicitPositionalEncoding(nn.Module):
    """Positional encoding."""
    # def __init__(self, num_hiddens, dropout, max_len=1000):
    def __init__(self,pretrain_snv_toks: Tokenised_SNVs, d_model: int, dropout: float = 0.0, max_len: int = 5000):
        ...
        self.P = torch.zeros((1, max_len, d_model))#1
        self.P =self.P.to(dtype=int)
        self.P = self.P.to(device)
        #self.P = torch.zeros_like((1, max_len, d_model))
        my_list = [0]
        pos_org = pretrain_snv_toks.pos_org
        for i in pos_org:
            my_list.append(i.item())
        
        X2 = torch.arange(max_len, dtype=torch.float32).reshape(
            -1, 1) / torch.pow(10000, torch.arange(
            0, d_model, 2, dtype=torch.float32) / d_model)
        #X = X.to(dtype=torch.float32)
        X = torch.tensor(my_list,dtype=torch.float32).reshape(
            -1, 1) / torch.pow(10000, torch.arange(
            0, d_model, 2, dtype=torch.float32) / d_model)
        #print(f'X = {X}')
        sin_x = torch.sin(X)
        ...
        #print(f'sin_x = {sin_x}')
        #print(f'cos_x = {sin_x}')
        self.P[:, :, 0::2] = sin_x # torch.sin(X)
        self.P[:, :, 1::2] = cos_x #torch.cos(X)
        pe = torch.zeros(max_len,d_model)
        for j,pos in enumerate(my_list):
        ...
        pe = pe.unsqueeze(0)
        print(self.P[0])
        px = torch.zeros((1, max_len, d_model))#1
        px[:, :, 0::2] = sin_x2 # torch.sin(X)
        px[:, :, 1::2] = cos_x2 # torch.sin(X)

        # for i,j in zip(self.P[0],px[0]):
        #     print (f'self = {i}')
        #     print(f' px = {j}')
        self.P = pe.to(device)
        print(f'self p after = {self.P[0]}')
    ...
    def forward(self, X, positions):
        ...
        # max_valid_index = self.P.shape[1] - 1
        # assert positions[0,:].max() <= max_valid_index, "Index out of bounds"
        # assert positions[0,:].min() >= 0, "Index out of bounds"



        temp = self.P[:, positions[0,:], :]
        
        X = X + temp #.to(X.device)
        return self.dropout(X)
