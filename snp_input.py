...
data_dir = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\pre_train\\dissertation\\'
gwas_dir = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\pre_train\\dissertation\\plink\\'
urate_file = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\pre_train\\dissertation\\'
cache_dir = "D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\cache\\new_cache\\"
...
class Tokenised_SNVs:
    def __init__(self, geno, encoding: int):
        ...
        # new code
        self.pos_org = positions + tmp
        sorted_pos = self.positions
        sorted_pos = sorted_pos.numpy()
        #counter = 0
        #max_diff = sorted_pos.max() - sorted_pos.min()
        snv_dic = {}
        #snv_dic_frac = {}
        #pos_frac = sorted_pos
        # for i in range(0,len(pos_frac)):
        #     pos_frac[i] = ((pos_frac[i]-pos_frac.min())/max_diff)
        # for i in range(0,len(pos_frac)):
        #     snv_dic_frac[i] = pos_frac[i-0]
        
        #--------------
        for i in range(0,(0+len(sorted_pos))):
            snv_dic[i] = sorted_pos[i-0]
        self.snv_dic = snv_dic
        snv_dic2_snv = { v: k for k, v in snv_dic.items() }
        for i in range(0,len(self.positions)):
            self.positions[i] = snv_dic2_snv[self.positions[i].item()]
...
def read_from_plink(parameters, remove_nan=False, subsample_control=True, encoding: int = 2, test_frac=0.3, verify_frac=0.05, summarise_genos=False):
    ...
    urate_tmp = pandas.read_csv(data_dir + '\\subjects.csv')
    ...
    phenos = urate_tmp
    usable_ids = urate_tmp['patient_id']
    del urate_tmp
    # avail_phenos = urate
    geno = geno_tmp
    del geno_tmp
    ...
    if (subsample_control):
        ...
        phenos = phenos[phenos["patient_id"].isin(sample_ids)]
    ...
    train_phenos = phenos[["cov_2", "cov_1", "bmi"]]
...
def get_train_test_verify_ids(phenos, parameters):
    ...
    set_cv_file = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\pre_train\\dissertation\\subjects.csv'
    set_cv = pandas.read_csv('D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\pre_train\\dissertation\\subjects.csv')
    if exists(set_cv_file):
        
        train_ids_list = set_cv[set_cv['set']==1]
        train_ids = train_ids_list['patient_id']
        train_ids_list = set_cv[set_cv['set']==3]
        test_ids = train_ids_list['patient_id']
        train_ids_list = set_cv[set_cv['set']==2]
        verify_ids = train_ids_list['patient_id']
        # test_ids = pandas.read_csv(test_ids_file)
        # verify_ids = pandas.read_csv(verify_ids_file)
    else:
        # usable_ids = phenos.eid.values
        gout = phenos["labels"].values
...
def get_pretrain_dataset(train_ids, params):
    ...
    if (os.path.exists(pt_pickle)):
        ...
        print("using data from:", data_dir)
        bed_file = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\pre_train\\dissertation\\plink\\'  + "merged_ad11_all.bed"
        bim_file = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\pre_train\\dissertation\\plink\\'  + "merged_ad11_all.bim"
        fam_file = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\pre_train\\dissertation\\plink\\'  + "merged_ad11_all.fam"
        print("bed_file:", bed_file)
        geno_tmp = read_plink1_bin(bed_file, bim_file, fam_file)
        geno_tmp["sample"] = pandas.to_numeric(geno_tmp["sample"])
        #withdrawn_ids = pandas.read_csv(data_dir + "w12611_20220222.csv", header=None, names=["ids"])
...
def get_train_test_verify(input_phenos, geno, pheno, parameters):
    ...
    gout = pheno["labels"].values
    # test_cutoff = (int)(math.ceil(test_split * geno.tok_mat.shape[0]))
    #print(f"the geno shape = {geno.tok_mat.shape}")
    ...
    age = input_phenos["cov_2"]
    bmi = input_phenos["bmi"]
    print('1')
    # 0 == Male, 1 == Female
    sex = torch.tensor(input_phenos["cov_1"].values == "1", dtype=torch.int64)
    ...
    # )
    print('1.3')
    print(f' len = {len(train_seqs)}')
    print(f' len = {len(train_phes)}')
    print(f' len = {len(train_pheno_vec)}')
    print(train_seqs)
    training_dataset = data.TensorDataset(
    ...
    print('exiting train_test_verify')
