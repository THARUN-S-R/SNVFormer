...
cache_dir = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer\\cache'
...
def main():
    ...
    os.chdir(home_dir + "D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer")
    pretrain_base = 'D:\\final_project\\ADUKBB_case_control_balanced\\ADUKBB_case_control_balanced\\PLINK'
...
def summarise_net(net, test_data, parameters, net_file):
    ...
    with torch.no_grad():
        for phenos, pos, tX, tY in test_iter:
            ...
            phenos_,pos_,tX_,tY_ =phenos, pos, tX, tY 
            #l.backward()
            
    #ph,seq = get_contributions(phenos_,pos_,tX_,tY_,net,parameters)
    ...
    #f1_score = metrics.f1_score(actual_vals, nn_scores)
    #print(f'F1 score = {f1_score}')
    reca = metrics.recall_score(actual_vals, predicted_vals)
    print(f'recall = {reca}')
    precise = metrics.precision_score(actual_vals, predicted_vals)
    print(f'precision = {precise}')
    confuse = metrics.confusion_matrix(actual_vals,predicted_vals)
    print(f'confusion matrix = {confuse}')
    


    
    fpr, tpr, _ = metrics.roc_curve(actual_vals, nn_scores)
    test_results.to_csv('D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\nets\\results.csv',index= False)
