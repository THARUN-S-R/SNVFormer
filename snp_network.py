...
def get_pretrained_encoder(state_file: str, params, snv_toks):
    ...
    encoder = nn.DataParallel(encoder, device)
...
def get_encoder_from_params(p: dict, pretrain_snv_toks):
    ...
    return get_encoder(p['encoder_size'], p['num_phenos'], pretrain_snv_toks.positions.max(), pretrain_snv_toks.num_toks, p['batch_size'], device, pretrain_snv_toks.string_to_tok['cls'], p)
...
def get_encoder(params, pretrain_snv_toks: Tokenised_SNVs):
    ...
    #print(f"value in tuple {max_seq_pos}")
    #max_seq_pos = int(max_seq_pos.item())
    max_seq_pos = len(pretrain_snv_toks.positions)
    print(f'type of max_seq_pos = {type(max_seq_pos)} and value is {max_seq_pos} and length = {type(max_seq_pos)}')
    encoder = Encoder(
        params,
        cls_tok,
        vocab_size,
...
def mask_sequence(seqs, pos, frac, tokenised_snvs: Tokenised_SNVs):
    ...
    #print(f'len of mask_tok = {(mask_tok)}')
    ...
    for seq in range(seqs.shape[0]):
        ...
        #print(f'seque = {seq_change_positions}')
        ...
        # mask_positions1.append(seq_mask_positions)
        ...
        #print(f'shape of mask_positions = {len(mask_positions)}')
        #print(f'shape of mask_positions1 = {(mask_positions1)}')
    # replace mask tokens
    #mask_positions_1 = mask_positions
    #mask_positions = np.transpose(np.array(mask_positions))
    # mask_positions1 = np.transpose(np.array(mask_positions))
    #print(f'shape of mask_positions out = {mask_positions.shape}')
    #print(f'mask_pos_indices = {mask_pos_indices}')
    #print(f'shape of mask_positions1 out = {mask_positions1.shape}')
    random_positions_1 = random_positions
    random_positions = np.transpose(np.array(random_positions))
    #mask_pos_indices = np.transpose(np.array(mask_pos_indices))
    # with open('D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\masks_for_ref.txt', 'w') as f:
    #     for line in mask_positions:
    #         f.write(f"{line}\n")
    #mask_positions.tofile('D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\masks_for_ref1.txt', sep=' ', format='%s')
    #mask_tensor = torch.from_numpy(mask_positions)
    #mask_positions = torch.clamp(mask_tensor,min =0 , max =( seqs.size(0) -1))
    #mask_positions = mask_tensor.numpy()
    #mask_positions.tofile('D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\masks_for_ref2.txt', sep=' ', format='%s')
    #print(f'values of mask_positions = {mask_positions}')
    #print(f'shape of mask_positions = {mask_positions.shape}')
    
    #print(f'values of seqs = {seqs}')
    #print(f'shape of seqs = {seqs.shape}')
    #print(f'size of seqs = {seqs.size(0)}')
    #print(f'mask_positions2 = {mask_positions[0]}')
    #print(f'mask_positions3 = {mask_positions[1]}')
    for z in mask_positions:
        seqs[z] = mask_tok
    for z in mask_pos_indices:
        pos[z] = -1
    ...
    #print(f'new_random_toks = {new_random_toks}')
    #print(f'new_random_toks_shape = {new_random_toks.shape}')
    #print(f'new_random_positions = {len(random_positions_1)}')

    for p,z in enumerate(random_positions_1):
        seqs[z] = new_random_toks[p]
    return seqs, pos
...
def pretrain_encoder(
    ...
    wip_nets_dir = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\nets\\'
    ...
    for e in range(num_epochs):
        ...
        for pos, seqs in tqdm(training_iter, ncols=0):
            ...
            # if (num_steps > 2):
            #     break
            #print(f'shape of seqs = {seqs.shape[0]}')
            #print(f'shape of seqs = {seqs.shape}')
            ...
            #print(f' chosen positions = {chosen_positions}')
        ...
        encoder_file = wip_nets_dir + get_net_savename(params) + ".encoder"
...
def train_everything(params=default_parameters):
    ...
    pt_lr = params['pt_lr']
    saved_nets_dir = 'D:\\final_project\\snvformer-icml22\\SNVFormer\\SNVFormer_ref\\nets\\'#D:\final_project\snvformer-icml22\SNVFormer\SNVFormer_ref\nets
    print('in train everything')
    

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print('in train everything 2')
    train_ids, train, test_ids, test, verify_ids, verify, geno, pheno, enc_ver = get_data(params)
    snv_toks = geno
    print('in train everything 3')
    net_dir = saved_nets_dir
    ...
    encoder_file = saved_nets_dir + get_net_savename(encoder_params) + ".encoder"
    ...
    if (train_new_encoder or not exists(encoder_file)):
        ...
        use_devices = ['cuda']
        encoder = nn.DataParallel(encoder, use_devices)
    ...
    if (continue_training):
        ...
        prev_net_name = saved_nets_dir + get_net_savename(prev_params)
        net.load_state_dict(torch.load(prev_net_name))
        new_net_name = saved_nets_dir + get_net_savename(params)
    else:
        prev_epoch = 0
    ...
    print("test dataset: ", check_pos_neg_frac(test))#verify
    ...
    train_net(net, train, verify, batch_size, num_epochs, device, lr, prev_epoch, test_frac, train_log_file, plink_base, snv_toks, params, subsample_input=False)
